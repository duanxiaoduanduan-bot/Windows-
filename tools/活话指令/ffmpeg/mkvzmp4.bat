...
if !total_count! equ 0 (
    echo 未找到任何.mkv格式文件，请将脚本放在MKV文件所在目录！
    pause
    exit /b 0
)

:: 5. 开始批量转换
echo 找到 !total_count! 个MKV文件，即将开始转换...
...
for /r %%f in (*.mkv) do (
    ...
    :: 6. 计算输出路径（保持原目录结构）
    :: 例：原路径 C:\Videos\Folder1\a.mkv → 输出 C:\Videos\ConvertedMP4\Folder1\a.mp4
